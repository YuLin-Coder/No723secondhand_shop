				...
				<main>
					<div class="main">
							...
							<div class="row">
								<div class="col-5">
									...
									<input type="hidden" name="id" value="${data.id }">
										...
										<div class="form-group">
											<label>物品分类</label> <select name="cname" class="form-control">
											<c:forEach items="${categoryList}" var="cate">
												<option value="${cate.name}" <c:if test="${cate.name == data.cname}">selected</c:if> >${cate.name}</option>
											</c:forEach>
										...
										<div class="form-group">
											<label>库存</label> <input type="number" name="stock" value="${data.stock }"
																	 class="form-control"> <small
												class="form-text text-muted"></small>
										</div>

										<div class="form-group">
											<label>价格</label> <input type="number" name="price" value="${data.price }"
																	 class="form-control"> <small
												class="form-text text-muted"></small>
										</div>
										...
										<div class="form-group">
											<label>物品简介</label> <input type="text" name="note" value="${data.note }"
																	   class="form-control"> <small
												class="form-text text-muted"></small>
										</div>
										...
										<div class="form-group">
											...
											<option value="9"  <c:if test="${data.discount == '9'}">selected</c:if>>9折扣</option>
											<option value="8"  <c:if test="${data.discount == '8'}">selected</c:if>>8折扣</option>
											<option value="7"  <c:if test="${data.discount == '7'}">selected</c:if>>7折扣</option>
											<option value="6"  <c:if test="${data.discount == '6'}">selected</c:if>>6折扣</option>
											<option value="5"  <c:if test="${data.discount == '5'}">selected</c:if> >5折扣</option>
										</select> <small class="form-text text-muted"></small>
										</div>
						...
						<div class="submit-box">
							...
							<a type="button" href="GoodsList" class="btn btn-outline-secondary">返回</a>
